from typing import Optional
from uuid import UUID
from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession
from pydantic import BaseModel

from app.core.database import get_db
from app.core.security import get_current_user
from app.models.content import Testimonial
from app.services.crud import CRUDService

router = APIRouter()
crud = CRUDService(Testimonial)


class TestimonialCreate(BaseModel):
    author_name: str
    author_position: Optional[str] = None
    author_company: Optional[str] = None
    author_image: Optional[str] = None
    content_uz: str
    content_ru: str
    content_en: str
    rating: int = 5
    sort_order: int = 0
    is_visible: bool = True


@router.get("")
async def list_testimonials(db: AsyncSession = Depends(get_db)):
    items = await crud.get_all(db, visible_only=True)
    return {"items": items}


@router.post("")
async def create_testimonial(data: TestimonialCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    return await crud.create(db, data.model_dump())


@router.put("/{id}")
async def update_testimonial(id: UUID, data: TestimonialCreate, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    item = await crud.update(db, id, data.model_dump())
    if not item:
        raise HTTPException(status_code=404, detail="Not found")
    return item


@router.delete("/{id}")
async def delete_testimonial(id: UUID, db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    success = await crud.delete(db, id)
    if not success:
        raise HTTPException(status_code=404, detail="Not found")
    return {"detail": "Deleted"}
